...
struct histogram {
	...
	void print(std::ostream& str) {
		for (size_t i = 0; i < data.size(); ++i) str << i << ":" << get(i) << endl;
		str << "total:" << accumulate(data.begin(), data.end(), 0, [](int sum, atomic<int> &value) { return sum + value.load(); }) << endl;
	}
...
int main(int argc, char **argv)
{
	...
	for (int t = 0; t < num_threads; ++t)
	{
		...
		threads.emplace_back([&]() { processed_count += worker(to - from, h, cfg).do_work(); }); // <-- execute this in a thread
	}
